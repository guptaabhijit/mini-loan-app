...
func (repository *UserRepository) GetUsersWithPaginationAndOrder(pagination scopes.GormPager, order scopes.GormOrderer) (users []models.User, totalCount int64, err error) {
	//err = repository.DB().Scopes(order.ToOrder("users", "id", "id", "created_at", "updated_at")).Count(&totalCount).Scopes(pagination.ToPaginate()).Find(&users).Error

	err = repository.DB().Model(&models.User{}).Scopes(order.ToOrder("users", "id", "id", "created_at", "updated_at")).Count(&totalCount).Scopes(pagination.ToPaginate()).Find(&users).Error
	//db.Model(&User{}).Where("uac.user_id = ?", "userId").Count(&count)

	//err = repository.DB().Scopes(order.ToOrder("users", "id")).Count(&totalCount).Find(&users).Error
	return
}
